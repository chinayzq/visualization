<template>
  <div class="finale-table-comp">
        ...
        >
          <template slot-scope="scope">
            ...
            <span v-if="scope.$index === 1" class="final-search-line">
              <!-- 如果搜索行有具名插槽，则渲染slot -->
              <slot
                v-if="datas.searchLineConfig[item.prop] && datas.searchLineConfig[item.prop].type === 'slot'"
                :name="datas.searchLineConfig[item.prop].slotName"
              ></slot>
              <searchLineItem v-else :datas="datas" :prop="item.prop" @eventEmit="filterEventEmit" />
            </span>
            <!-- 表格内容 -->
            <span v-else>
              <!-- 通过prop在headerConfig中匹配 -->
              <slot
                v-if="datas.detailConfig[item.prop] && datas.detailConfig[item.prop].type === 'slot'"
                :name="datas.detailConfig[item.prop].slotName"
                :datas="scope.row"
              ></slot>
              <span v-else-if="datas.detailConfig[item.prop]">
                <detailLineItem :datas="datas" :prop="item.prop" :row="scope.row" @eventEmit="detailEventEmit" />
              </span>
              <span v-else>
                {{ scope.row[item.prop] || "-" }}
              </span>
    ...
    />
    <!-- 高级筛选 -->
    <el-drawer v-if="AdvanceFilterShow" title="高级筛选" :size="586" :visible.sync="AdvanceFilterShow">
      <div class="advance-filter-container">
        <div class="form-container">
          <el-form ref="advanceFilterForm" :model="advanceFilterForm" label-width="120px" label-position="left">
            <span v-for="(item, index) in Object.keys(datas.advanceFilterConfig)" :key="index">
              ...
              <span v-if="datas.searchLineConfig[item]">
                <el-form-item :label="datas.searchLineConfig[item].label">
                  ...
                  <span v-if="datas.searchLineConfig[item].type === 'input'">
                    <el-input placeholder="" v-model="advanceFilterForm[item]" clearable></el-input>
                  </span>
                  <!-- input类型 -->
                  <span v-if="datas.searchLineConfig[item].type === 'numberInput'">
                    <el-input
                      placeholder=""
                  ...
                  <!-- selector类型 -->
                  <span v-else-if="datas.searchLineConfig[item].type === 'selector'">
                    <el-select
                      style="width: 100%"
                      v-model="advanceFilterForm[item]"
                      placeholder=""
                      @change="selectorLinkageHandler(item, advanceFilterForm[item])"
                      clearable
                    >
                      <el-option
                        v-for="(item, index) in datas.searchLineConfig[item].optionList"
                        :value="item.value"
                        :label="item.label"
                        :key="index"
                      ></el-option>
              ...
              <span v-else>
                <el-form-item :label="datas.advanceFilterConfig[item].label">
                  <!-- 时间段选择类型（开始时间 - 结束时间） -->
                  <span v-if="datas.advanceFilterConfig[item].type === 'dateRange'">
                    <el-date-picker
                      style="width: 192px"
                      v-model="advanceFilterForm[item][0]"
                      :type="datas.advanceFilterConfig[item].pickerType || 'datetime'"
                      placeholder="选择开始时间"
                    ></el-date-picker>
                  ...
                  <span v-else-if="datas.advanceFilterConfig[item].type === 'tagList'">
                    <el-select
                      ref="tagListRef"
                  ...
                  <span v-else-if="datas.advanceFilterConfig[item].type === 'input'">
                    <el-input
                      v-model="advanceFilterForm[item]"
...
<script>
import searchLineItem from "./searchLineItem.vue"
import detailLineItem from "./detailLineItem.vue"
...
export default {
  ...
  data() {
    return {
      ...
      listQuery: {
        ...
        rowCount: 20,
        resultMode: 2,
        tagType: this.datas.advanceFilterConfig.tagList && this.datas.advanceFilterConfig.tagList.tagType,
        tagName: null,
      },
    }
  },
  ...
  methods: {
    ...
    tagListChange() {
      ...
      this.tagOptions = this.tagOptionsBak
    },
    // saveFlag: 是否缓存当次查询
    getTagList(query, saveFlag) {
      ...
      getTagList(this.listQuery)
        .then((res) => {
          ...
          saveFlag && (this.tagOptionsBak = this.tagOptions)
    ...
    freshCount() {
      ...
      this.datas.searchLineConfig.operation.filterCount = count > 0 ? count : ""
    },
    ...
    advanceFilterHandler() {
      ...
      this.$emit("tableEventHandler", {
        type: "goSearch",
        params: JSON.parse(JSON.stringify(this.advanceFilterForm)),
      })
      this.AdvanceFilterShow = false
    },
    selectorLinkageHandler(item, value) {
      // 触发列表事件
      this.$emit("tableEventHandler", {
        type: "selectorLinkage",
    ...
    advanceFormReset() {
      for (let key in this.advanceFilterForm) {
        console.log("key", key)
        if (this.advanceFilterForm[key] instanceof Array) {
          let tempList = []
          this.advanceFilterForm[key].forEach((item) => {
            tempList.push(null)
          })
          this.advanceFilterForm[key] = key === "tagList" ? [] : tempList
          console.log("xxx2", this.advanceFilterForm[key])
        } else {
      ...
      // this.$emit("tableEventHandler", {
      //   type: "selectorLinkage",
      //   params: { item: "industryType", value: "" },
      // });
      console.log("xxx", this.advanceFilterForm)
    },
    // 隐藏第一个selection
    ...
    filterLineToggle(action) {
      ...
      if (this.datas.selection && this.datas.selection.need) {
    ...
    tableRowClassName({ row, rowIndex }) {
      if (rowIndex === 0 || (rowIndex === 1 && !this.filterLineShow)) {
        ...
        if (this.rowClassNameCB && rowIndex > 1) return this.rowClassNameCB(row, rowIndex)
        return ""
      }
    },
    onSelectAll() {
      this.datas.selection.singleSelect && this.$refs.table.clearSelection()
    },
    // 存储勾选的rowList
    selectedChange(list) {
      if (this.datas.selection.singleSelect && list.length > 1) {
        const newRows = list.filter((it, index) => {
      ...
      list = list.filter((item) => {
        return !(
          (Object.keys(item).length === 1 && item.enableStatus && item.enableStatus === "DISABLE") ||
          JSON.stringify(item) === "{}"
        )
      })
      console.log("table选择的数据：", list)
      this.$emit("tableEventHandler", {
        ...
        if (datas.eventName === "showMoreFilters") {
          ...
          Object.keys(this.advanceFilterForm).forEach((item) => {
            tempObj[item] = this.datas.searchLineConfig[item]
              ...
              : JSON.parse(JSON.stringify(this.datas.advanceFilterConfig[item].value))
          })
...
.finale-table-comp {
  ...
  /deep/ .el-table__body-wrapper.is-scrolling-none > table > tbody > tr:nth-child(2):hover {
    .el-table__cell {
      background-color: #fff;
    }
  }
